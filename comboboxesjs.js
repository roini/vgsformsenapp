...
let selectViaVilages = document.getElementById('CityAndVilages');

//let desiredCity = ""; // Specify the desired city here
let inCaseWifi = [
...
let inCasefiber=['აირჩიეთ რაიონი','სამტრედიის რაიონი','ვანის რაიონი','აბაშის რაიონი','ლანჩხუთის რაიონი'];
...
let regionWithVilagesFiber = [
	...
	function ChangeregionsOP() {
		...
		regionOpt=document.getElementById('regions').value;
		selectViaVilages.innerHTML = ' '; // Clear the options
		detectTypeInt(typeOfIntTech,regionOpt);
			function detectTypeInt(typeofint,desiredCity){
			       if(typeofint=='ოპტიკური ინტერნეტი'){
					   for (let i = 0; i < regionWithVilagesFiber.length; i++) {
							if (regionWithVilagesFiber[i].city === desiredCity) {
								
								
								for (let j = 0; j < regionWithVilagesFiber[i].villages.length; j++) {
								    let option = document.createElement('option');
									option.value = regionWithVilagesFiber[i].villages[j];
									option.text = regionWithVilagesFiber[i].villages[j];
									selectViaVilages.appendChild(option);
									desiredCity='';
								}
								
								break; // Exit the loop after finding the desired city
							}
				        }
				   }
				   
				   else if(typeofint=='უკაბელო ინტერნეტი(საჰაერო)'){
					   for (let i = 0; i < regionWithVilagesWifi.length; i++) {
							if (regionWithVilagesWifi[i].city === desiredCity) {
								
								for (let j = 0; j < regionWithVilagesWifi[i].villages.length; j++) {
									let option = document.createElement('option');
									option.value = regionWithVilagesWifi[i].villages[j];
									option.text = regionWithVilagesWifi[i].villages[j];
									selectViaVilages.appendChild(option);
									desiredCity='';
								}
								console.log("-------------------");
                   ...
                   else if(desiredCity=="აირჩიეთ რაიონი"){
                    let option = document.createElement('option');
                    option.value = desiredCity;
                    option.text = desiredCity;
                    selectViaregions.appendChild(option);
                   }
